...
CREATE TABLE IF NOT EXISTS `st_driver_hours` (
    ...
    -- Bewusst TINYINT (nicht TINYINT(1)) - ist ein 3-Zustands-Zähler
    -- (0/1/2, siehe server/sv_hours.lua), TINYINT(1) würde oxmysql beim
    -- Lesen automatisch zu einem verlustbehafteten Lua-Boolean casten.
    `warned_continuous` TINYINT UNSIGNED NOT NULL DEFAULT 0,
    `warned_daily` TINYINT UNSIGNED NOT NULL DEFAULT 0,
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`driver_id`),
